...
from luthiertext.word_vectors import word_by_word_cooccurrence as wwc
...
def test1():
	...
	X, vocab = wwc(sample_corpus, window=None, left_n=2, right_n=2, tokenizer=tokenizer, 
...
def test2():
	...
	zamba_vec = np.zeros(len(vocab))
	for tk,freq in zamba_dict.items():
		zamba_vec[vocab[tk]] = freq
...
def test3():
	...
	try:
		assert np.array_equal(X.diagonal().reshape(-1),np.array(list(freq_dict.values())))
		print('Test 3 passed!')
	except AssertionError:
...
def test5():
	...
	try:
		for tk in vocab.keys():
			assert X[vocab[tk],vocab[tk]] in (1.,2.)
		assert len(vocab) == 5
		print('Test 5 passed!')
	except AssertionError:
